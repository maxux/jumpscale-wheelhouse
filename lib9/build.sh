...
for pybin in $(ls -1d /opt/python/*{35,36}*/bin | grep -v cpython); do
    "${pybin}/pip" install --upgrade cython

    export CXXFLAGS="-DKJ_USE_EPOLL=0 -D__BIONIC__=1"

    # preinstall pycapnp with specific build options
    "${pybin}/pip" install pycapnp
    "${pybin}/pip" wheel pycapnp -w /io/wheelhouse/repository/
...
# temp fix
sed -i '/^JumpScale/d' requirements.txt
